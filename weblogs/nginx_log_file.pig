...
RAW_LOGS = LOAD '/user/orenault/weblogs.log' as (line:chararray);
...
INFO = FOREACH LOGS_BASE GENERATE myfuncs.extract_url_info(url) as url_info:(url:chararray, nom_jvm:chararray, t_done:long, t_resp:long, t_page:long);;
ALL_RESULTS = FOREACH INFO GENERATE url_info.url as url:chararray, url_info.nom_jvm as nom_jvm:chararray, (long)url_info.t_done as t_done, (long)url_info.t_resp as t_resp, (long)url_info.t_page as t_page;
